        ...
        {rows.map((row) => (
          <div className="space-y-1.5" key={row.milestoneId}>
            <div className="flex items-baseline justify-between gap-3 text-sm">
              <span className="font-medium text-[var(--color-text-primary)]">{row.milestoneTitle}</span>
              <span className="shrink-0 font-semibold text-[var(--color-text-primary)]">{formatCurrency(row.totalAmount)}</span>
            </div>
